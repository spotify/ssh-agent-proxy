 ...
 * TBA
 */
class IdentityIterator implements Iterator<Identity> {

  private static final Logger log = LoggerFactory.getLogger(IdentityIterator.class);

  private final Iterator<byte[]> byteIterator;

  IdentityIterator(final byte[] bytes) {
  ...
  public Identity next() {
    ...
    while (identity == null && byteIterator.hasNext()) {
      final byte[] keyBlob = byteIterator.next();
      final byte[] keyComment = this.byteIterator.next();
      try {
        identity = Identity.from(keyBlob, new String(keyComment));
