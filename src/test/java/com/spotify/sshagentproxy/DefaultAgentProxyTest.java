...
public class DefaultAgentProxyTest {
  ...
  public void testList() throws Exception {
    ...
    final Iterator<Identity> identities = new IdentityIterator(answerData);
    when(in.readIdentitiesAnswerData2(eq(answer))).thenReturn(identities);

    try (final DefaultAgentProxy proxy = new DefaultAgentProxy(out, in)) {
      final List<Identity> ids = proxy.list();
    }
  }
